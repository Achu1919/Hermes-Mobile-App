...
import { Check, ChevronRight, Cpu, Info, SlidersHorizontal, X } from 'lucide-react'

import { loadModelOptions, loadProfileDetails, setProfileModel, type LiveProfile, type LiveSession, type ModelOptions, type ProfileDetails } from '../hermes'
...
export function BotProfileSheet({ profile, session, onClose, onUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([loadProfileDetails(session.profile), loadModelOptions(session.profile)])
      .then(([nextDetails, nextOptions]) => { if (active) { setDetails(nextDetails); setModelOptions(nextOptions) } })
