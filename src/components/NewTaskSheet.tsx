...
export function NewTaskSheet({ profiles, onClose, onCreated }: Props) {
  ...
  useEffect(() => { if (!selectedBlueprint) return; const seeded: Record<string, string> = {}; selectedBlueprint.fields.forEach(field => { seeded[field.name] = field.name === 'deliver' && (!field.default || field.default === 'origin') ? 'local' : field.default || '' }); setValues(seeded) }, [selectedBlueprint])
  const submit = async () => {
    setSaving(true); setError('')
    try {
      if (!bot) throw new Error('Choose a Bot for this task.')
      if (selectedBlueprint) { await instantiateCronBlueprint(bot, selectedBlueprint.key, { ...values, deliver: values.deliver || deliver.join(',') || 'local' }) }
      else {
        ...
        await createCronJob(bot, { name: name.trim() || undefined, prompt: prompt.trim(), schedule: schedule.trim(), deliver: deliver.join(',') || 'local', ...(model ? { model, provider } : {}) })
      }
  ...
  return <main className="app task-create-sheet"><header className="task-create-head"><button className="round-control" onClick={onClose} aria-label="Close new task"><ArrowLeft size={18}/></button><span><b>New task</b><small>Schedule an automated Hermes prompt</small></span><button className="round-control" onClick={onClose} aria-label="Close new task"><X size={17}/></button></header><div className="task-create-scroll">
    ...
    <FormLabel title="Bot" hint="The task will be stored in this Bot’s cron schedule."><select value={bot} onChange={event => setBot(event.target.value)}>{profiles.map(profile => <option key={profile.name} value={profile.name}>{profile.display_name || profile.name}</option>)}</select></FormLabel>
    ...
    <section className="task-form-section"><h3>Deliver to</h3><div className="task-delivery-list">{(targets.length ? targets : [{ id: 'local', name: 'This desktop', home_target_set: true }]).map(target => { const selected = selectedBlueprint ? (values.deliver || 'local').split(',').includes(target.id) : deliver.includes(target.id); return <label key={target.id}><input type="checkbox" checked={selected} onChange={() => selectedBlueprint ? setValues(current => ({ ...current, deliver: target.id })) : toggleDelivery(target.id)} /><span><b>{target.name}</b>{!target.home_target_set && <small>Home channel not configured</small>}</span>{selected && <Check size={15}/>}</label> })}</div></section>
