...
export function CapabilityManager({ profile, session, onBack, onUpdated }: Props) {
  ...
  return <main className="app management-sheet">
    ...
    {hubResults && <section className="capability-group"><div className="capability-group-head"><span>Skills Hub results</span><small>{hubResults.length}</small></div>{hubResults.length ? hubResults.map(item => <div className="capability-row" key={item.name}><Wrench size={17}/><span><b>{item.name}</b><small>{item.description || 'Installable Hermes skill'}</small></span>{skills.some(skill => skill.name === item.name) ? <em>Installed</em> : <button className="capability-action" disabled={Boolean(installing)} onClick={() => void install(item.name)}>{installing === item.name ? 'Adding…' : 'Add'}</button>}</div>) : <p className="management-empty">No Skills Hub results matched “{query}”.</p>}</section>}
    <section className="capability-group"><div className="capability-group-head"><span>Installed skills</span><small>{enabledSkills}/{skills.length} enabled</small></div>{loading && !details ? <p className="management-empty">Loading capabilities…</p> : visibleSkills.length ? visibleSkills.map(item => <CapabilityRow item={item} key={item.name} onToggle={() => toggle(setSkills, item.name)}/>) : <p className="management-empty">No installed skills match this search.</p>}</section>
    <section className="capability-group"><div className="capability-group-head"><span>Toolsets</span><small>{enabledToolsets}/{toolsets.length} enabled</small></div><p className="capability-note">Toolset changes are saved to this Bot and apply to a new or reset conversation.</p>{visibleToolsets.length ? visibleToolsets.map(item => <CapabilityRow item={item} key={item.name} onToggle={() => toggle(setToolsets, item.name)}/>) : <p className="management-empty">No toolsets match this search.</p>}</section>
  </main>
}
